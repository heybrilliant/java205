...
select ename, substr(hiredate,1,5) as hireyearandmonth
from emp
;
...
select ename,hiredate, to_char(hiredate, 'YY/MM DY')
from emp;


-- 20. 올해 몇 칠이 지났는지 출력하시오. 현재날짜에서 올해 1월 1일을 뺀 결과를 출력하고 TO_DATE 함수를 사용하여 데이터 형을 일치 시키시오.
select sysdate as "오늘 날짜",
trunc(sysdate - to_date('2021-01-01','YYYY-MM-DD')) as "며칠이 지났을까요"
from dual
;

-- 21. 사원들의 상관 사번을 출력하되 상관이 없는 사원에 대해서는 NULL 값 대신 0으로 출력하시오.
select ename, nvl(mgr,0) as mgrnum
from emp
;
...
select max(sal) as "급여 최고액", min(sal) as "급여 최저액", sum(sal) as "급여 총액", round(avg(sal)) as "급여 평균"
from emp
;

-- 24. 각 담당 업무 유형별로 급여 최고액, 최저액, 총액 및 평균 액을 출력하시오. 평균에 대해서는 정수로 반올림 하시오.
select job, max(sal) as "급여 최고액", min(sal) as "급여 최저액", sum(sal) as "급여 총액", round(avg(sal)) as "급여 평균"
from emp
group by job
;
...
select deptno, 
    ...
    decode(deptno,10,'NEW YORK',
                    ...
                    30,'CHICAGO',
                    40,'BOSTON') as "지역명",
        count(*) as "사원수", round(avg(sal)) as "부서 평균 급여"
from emp
group by deptno
;
