...
<body>
	...
	<jsp:useBean id="product" class="product.Product" scope="session"></jsp:useBean>
	<%-- 1. session 객체의 product 이름의 속성이 있는지 확인
2. 있다 -> 현재 product 이름의 변수에 session 영역에 저장되어있는 객체를 저장해서 사용
3. 없다 -> product.Product 클래스로 객체 생성하고 session 영역의 속성에 객체를 저장한다 - product 이름으로 저장한다 
